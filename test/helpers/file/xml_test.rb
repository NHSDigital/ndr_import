...
class XmlTest < ActiveSupport::TestCase
  ...
  test '.import_xml_file should accept safepath' do
    ...
    assert_equal 2, doc.children.reject{ |c| c.text =~ /\A\n *\Z/}[0].children.reject{ |c| c.text =~ /\A\n *\Z/}.length
    assert_equal "note", doc.children.reject{ |c| c.text =~ /\A\n *\Z/}[0].children.reject{ |c| c.text =~ /\A\n *\Z/}[0].name
    assert_equal 3, doc.children.reject{ |c| c.text =~ /\A\n *\Z/}[0].children.reject{ |c| c.text =~ /\A\n *\Z/}[0].children.reject{ |c| c.text =~ /\A\n *\Z/}.length
    assert_equal "Thu Dec 13 13:12:00 UTC 2012", doc.children.reject{ |c| c.text =~ /\A\n *\Z/}[0].children.reject{ |c| c.text =~ /\A\n *\Z/}[0].children.reject{ |c| c.text =~ /\A\n *\Z/}[0].text
